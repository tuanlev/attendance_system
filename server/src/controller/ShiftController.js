...
exports.getShiftById = async (req, res, next) => {
    try {
         if (!req.isAuthenticated && req.role !=  require("../security/JwtUtils").Option.ADMIN)
            throw new ErrorHandling(403, "You do not have permission");
        const data = await ShiftService.getShiftById({ shiftId: req.params.shiftId });
        res.status(StatusCodes.OK).json({
            success: true,
            data
        });
...
exports.updateShiftById = async (req, res, next) => {
    try {
         if (!req.isAuthenticated && req.role !=  require("../security/JwtUtils").Option.ADMIN)
            throw new ErrorHandling(403, "You do not have permission");
        const data = await ShiftService.updateShiftById({ shiftId: req.params.shiftId }, req.body);
        res.status(StatusCodes.OK).json({
            success: true,
...
exports.deleteShiftById = async (req, res, next) => {
    try {
         if (!req.isAuthenticated && req.role !=  require("../security/JwtUtils").Option.ADMIN)
            throw new ErrorHandling(403, "You do not have permission");
        const result = await ShiftService.deleteShiftById({ shiftId: req.params.shiftId });
        res.status(StatusCodes.OK).json({
            success: true,
            ...result
        });
